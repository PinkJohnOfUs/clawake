...
- Admin-only setup RPC, exact origin checks, one-use POST tickets, state, browser cookie and PKCE.
- Short-lived bounded in-memory transactions; no code exchange through agent chat.
- Atomic private token replacement and rejection of absent/unexpected scope grants.
- Configurable publicOrigin for loopback now and HTTPS behind protected ingress later.
- SETUP.md covers new Web-client credentials, separate token storage, install and rollback.
- Mocked OAuth security tests and a browser smoke test; Google consent remains a user action.
